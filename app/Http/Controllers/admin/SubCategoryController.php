...
class SubCategoryController extends Controller
{
    ...
    public function index(Request $request){
      ...
      $sub_categories = \DB::table('sub__categories') 
      ->select('sub__categories.*','categories.name as categoryName')
      ->latest('sub__categories.id')
      ->leftJoin('categories','categories.id','sub__categories.category_id'); 


      if(!empty($request->get('keyword'))){
          $sub_categories = $sub_categories->where('sub__categories.name', 'like', '%'.$request->get('keyword').'%');
...
public function update($sub_category_id,Request $request){
  ...
  if(empty($sub_category)){
     ...
     $request->session()->flash('success','Sub_Category updated successfully');
     return response()->json([
      'status' => true,
